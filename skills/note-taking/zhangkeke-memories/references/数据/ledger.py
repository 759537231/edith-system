...
PAIRS_PATH    = os.path.join(os.path.dirname(__file__), 'reimburse_pairs.json')
RULES_PATH    = os.path.join(os.path.dirname(__file__), 'correction_rules.json')
...
def _infer_in_budget(note: str) -> bool | None:
...
def _record_correction(note: str, in_budget: bool | None):
...
def add_entry(
    ...
    source: str = 'manual',
    reimburse_pair_id: str | None = None,
    **extra
) -> dict:
    """
    添加一条账目（v2）。
...
def _apply_correction_rules(note: str) -> dict | None:
    rules = _load_rules()
...
def correct_entry(entry_id: str, **updates) -> dict | None:
    """
    纠正条目，同时触发学习。
...
def add_expense_with_reimburse(
    ...
    date: str,
    merchant: str,
    reimburse_amount_yuan: float | None = None,
    etype: str = 'expense',
    category: str = '📦 其他',
    note: str = '',
    source: str = 'manual',
...
def get_summary(month: str | None = None) -> dict:
    ...
    remaining = budget - sum(e['amount'] for e in in_b)  # 支出存为正数，所以要减
